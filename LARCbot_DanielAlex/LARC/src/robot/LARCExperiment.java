...
/**
 * Experiment program that does some of the things that might be important when running an experiment. It runs an agent on the environment and periodically asks the agent to
 * "freeze learning": to stop updating its policy for a number of episodes in order to get an estimate of the quality of what has been learned so far.
 *
 * The experiment estimates statistics such as the mean and standard deviation of the return gathered by the policy and writes those to a comma-separated value file called
 * results.csv.
 *
 * This experiment also shows off some other features that can be achieved easily through the RL-Glue env/agent messaging system by freezing learning (described above), having the
 * environment start in specific starting states, and saving and loading the agent's value function to/from a binary data file.
 * 
 * @author Brian Tanner
 */
public class LARCExperiment extends Thread {

	LARCRobot myRobot;

	public LARCExperiment(LARCRobot myRobot) {
	...
	/**
	 * Tell the agent to stop learning, then execute n episodes with his current policy. Estimate the mean and variance of the return over these episodes.
	 * 
	 * @return
	 */
	...
	// for (i = 0; i < n; i++) {
	// /*
	// * We use a cutoff here in case the policy is bad and will never end an episode
	// */
	...
	public void runExperiment() {
		...
		// System.out
		// .println("After Episode\tMean Return\tStandard Deviation\n-------------------------------------------------------------------------");
		// offlineDemo();
		...
		//
		// System.out
		// .println("Evaluating the agent's default policy:\n\t\tMean Return\tStandardDeviation\n------------------------------------------------------");
		...
		//
		// System.out
		// .println("Evaluating the agent after loading the value function:\n\t\tMean Return\tStandardDeviation\n------------------------------------------------------");
		...
		// System.out
		// .println("Evaluating the agent a few times from a fixed start state of 2,3:\n\t\tMean Return\tStandardDeviation\n-------------------------------------------");
		// thisScore = evaluateAgent();
		// printScore(0, thisScore);
		//
		// System.out
		// .println("Evaluating the agent again with the random start state:\n\t\tMean Return\tStandardDeviation\n------------------------------------------------------");
	...
	// LARCExperiment theExperiment = new LARCExperiment();
